...
try:
  ...
  # You need to write the pathway and the file that you want to add on the back.
  file_path = r'C:\MyWorkspace\upload\text.txt'

  # 파일 입출력을 실행하는 내장 함수 open()
  # 반환값으로 파일 입출력을 담당하는 객체가 리턴됩니다.
  # 첫번째 인수로 파일 경로를, 두번째 인수로 모든를 설정
  f = open(file_path, 'w')
  ...
  # 반드시 사용 후 자원을 해제하여 메모리 누수를 방지합니다.
  f.close()
